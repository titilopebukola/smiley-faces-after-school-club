...
login.onclick = () => {
  if (input[0].value != "" && input[1].value != "") {
    if (input[0].value == localStorage.setItem("username") && input[0].value == localStorage.getItem("pasword")) {
      form.onsubmit = () => {
        return 1;
      };
    } else {
      ...
      input[0].nextElementSibling.textContent = "Username NOT match empty";
      setTimeout(() => {
    ...
    input[1].nextElementSibling.textContent = "Password NOT match empty";
    setTimeout(() => {
